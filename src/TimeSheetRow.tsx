...
export function TimeLogRow({ row, number, setPayload: setAction }: TimeLogRowProps) {
  ...
  let signInButtonClass = row.signInTime ? "btn btn-success btn-sm disabled" : "btn btn-primary btn-sm";
  let signOutButtonClass = row.signInTime ? "btn btn-primary btn-sm mx-2" : "btn btn-primary btn-sm disabled mx-2";
  let resetButtonClass = row.signInTime ? "btn btn-warning btn-sm" : "btn btn-warning btn-sm disabled";

  if (!display) {
    signInButtonClass = signInButtonClass + ' d-none';
    signOutButtonClass = signOutButtonClass + ' d-none';
    resetButtonClass = resetButtonClass + ' d-none';
  }

  return (
    <tr onMouseEnter={() =>setDisplay(!display)} onMouseLeave={() =>setDisplay(!display)}>
      <td>{number}.</td>
      <td>{row.firstName}</td>
      <td>{row.lastName}</td>
